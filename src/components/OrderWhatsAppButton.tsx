...
} from "@/lib/domain/orderWhatsApp";
import { openWhatsAppMessage } from "@/lib/domain/whatsappWeb";

export function OrderWhatsAppButton(input: OrderWhatsAppInput) {
  const [error, setError] = useState<string | null>(null);
  const waDisabledReason = orderWhatsAppDisabledReason(input);
  const waLinks = orderWhatsAppLinks(input);
  ...
  return (
    <>
      ...
      <a
        ...
        aria-label={
          waLinks
            ? `WhatsApp ${input.purchaserName ?? "purchaser"}`
            : (waDisabledReason ?? "WhatsApp unavailable")
        }
        tabIndex={waLinks ? undefined : -1}
        onClick={(e) => {
          e.preventDefault();
          const links = orderWhatsAppLinks(input);
          if (!links) {
            setError(
              waDisabledReason ?? "WhatsApp is unavailable for this row.",
            );
            return;
          }
        ...
        title={
          waLinks
            ? "Open WhatsApp with order acceptance message"
            : (waDisabledReason ?? "WhatsApp unavailable")
        }
      >
